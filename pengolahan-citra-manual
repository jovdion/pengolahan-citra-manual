...
def rotate_image(pixels, width, height, angle=90):
    if angle == 90:
        return [[pixels[height - x - 1][y] for x in range(height)] for y in range(width)], height, width
    elif angle == 180:
        return [[pixels[height - y - 1][width - x - 1] for x in range(width)] for y in range(height)], width, height
    elif angle == 270:
        return [[pixels[x][width - y - 1] for x in range(height)] for y in range(width)], height, width
